...
Galene can import transactions from a **CSV file** you fill out — useful when a bank isn’t connected yet or you have history from another tool. You can also **export** the current filtered set (or the full set from Settings → Data) as CSV with the same columns.
...
1. On **Transactions**, use the **Export CSV** icon (arrow out of a tray). The download includes every transaction that matches the **current filters**, across pages, not only the rows on screen.
2. Clear filters first if you want the full list from that page.
3. On **Settings → Data**, the same export icon on the Transactions row downloads the **full** set (no filters).

The file is UTF-8 CSV named like `galene-transactions-YYYY-MM-DD.csv`. Export is one row per transaction. Split parts are not extra columns; the primary category name is used.

You need to be signed in. The file does not include bank secrets, provider ids, or external ids.

## Columns

These columns are the import template and the export header.
