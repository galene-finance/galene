...
Single sign-on, when enabled, is app-native: Galene redirects to the identity provider and sets its own session cookie. Do not put Authentik forward-auth or oauth2-proxy in front of the app for that flow. See [Single sign-on (OIDC)](/self-host/oidc/). The redirect URI uses the public origin these proxy headers reconstruct.
