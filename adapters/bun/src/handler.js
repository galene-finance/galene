...
function get_origin(request, url) {
	// default to the protocol the request arrived on; set PROTOCOL_HEADER (e.g.
	// x-forwarded-proto) when TLS terminates at a reverse proxy upstream
	const protocol = decodeURIComponent(
		(protocol_header && request.headers.get(protocol_header)) || url.protocol.replace(':', '')
	);
