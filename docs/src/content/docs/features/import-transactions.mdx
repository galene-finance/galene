...
3. On the export page, change filters (they apply as you go; there is no Filter button). Selected accounts, categories, and tags are pills with ×. Check the columns to include (all seven on by default; at least one required), then **Export CSV**. The file matches the matching-count on the page.
