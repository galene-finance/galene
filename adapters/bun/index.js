		...
		async adapt(builder) {
			...
			const virtual_files = {
				[manifest_file]:
					...
					// Kit's default prerender origin is a placeholder; only pin the request
					// origin when the user explicitly configured one, otherwise fall back to
					// the per-request origin (a pinned placeholder would fail CSRF origin checks)
					`export const origin = ${
						builder.config.kit.prerender.origin === 'http://sveltekit-prerender'
							? 'undefined'
							: JSON.stringify(builder.config.kit.prerender.origin)
					};`,
